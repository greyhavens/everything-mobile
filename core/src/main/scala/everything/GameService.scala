...
trait GameService {
  ...
  def flipCard (gridId :Int, pos :Int, expectCost :Int) :RFuture[FlipCardResult]

  /** Sells the specified card. */
  def sellCard (thingId :Int, created :Long) :RFuture[SellCardResult]
  ...
  def openGift (thingId :Int, created :Long) :RFuture[GiftInfo]

  /** Fetches info for this player's shop. */
  def getShopInfo () :RFuture[ShopInfo]
